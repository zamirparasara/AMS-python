...
def authorize(event):
    ...
    flag = 0
    for line in fr:
        if student_username_entry.get() in line and student_password_entry.get()=='pass':
            flag = 1
            break
    if flag == 1 and len(student_username_entry.get())==6:
        student_portal()
    fr.close()
...
def admin_record():
    ...
    optionList = ('PYTHON', 'DBMS', 'DS', 'JAVA', 'NEN', 'DCN')
...
img23=PhotoImage(file='logo-eAttendance.png')
